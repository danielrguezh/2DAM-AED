...
1.  Crear `proxy.conf.json` en la raíz:
    ...
    ```
2.  Registrarlo en `angular.json` (sección `architect > serve > options`):
    ```json
    "proxyConfig": "proxy.conf.json"
    ```

### 1.3 Variables de Entorno
En `src/environments/environment.ts`, definimos la URL base relativa para que use el proxy:
```typescript
...
### 2.1 Modelos de Datos (`auth.model.ts`)
...
### 2.2 Servicio de Autenticación (`auth.service.ts`)
Encargado de la lógica de negocio de sesión.
- **`login(credentials)`**: Realiza `POST /auth/login`. Si es exitoso, guarda el token.
- **Gestión del Token**: Se almacena en `localStorage` (`setItem('auth_token', token)`).
- **`logout()`**: Elimina el token de `localStorage` y redirige a `/login`.
- **`isLoggedIn()`**: Retorna `true` si existe un token almacenado.

### 2.3 Interceptor HTTP (`auth.interceptor.ts`)
Intercepta **todas** las peticiones salientes para inyectar las credenciales.
1.  Obtiene el token del `AuthService`.
2.  Si existe, clona la petición HTTP y añade la cabecera:
    `Authorization: Bearer <token_jwt>`
3.  Manejo de errores: Si el servidor devuelve **401 Unauthorized**, fuerza el `logout` para evitar estados inconsistentes.

**Registro**: Se añade en `app.config.ts` dentro de `provideHttpClient(withInterceptors([authInterceptor]))`.

### 2.4 Guardia de Rutas (`auth.guard.ts`)
...
### 3.1 Página de Login
- Formulario reactivo (`ReactiveFormsModule`) con campos `username` y `password`.
...
- Configurada en `app.routes.ts` con `canActivate: [authGuard]`.
- Utiliza `TasksApiService` para obtener la lista de tareas.
